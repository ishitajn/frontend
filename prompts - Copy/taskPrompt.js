...
export function buildTaskPrompt(instructions) {
    // *** FIX 1: Expect the nested conversationAnalysis object and destructure from it. ***
    const { goal, flirtyValue, lengthValue, endWithQuestion, linguisticStyle, strictGoalOverride, forceNewTopic, myName, theirName, emojiStrategy, conversationBreakDetected, conversationAnalysis } = instructions;

    // *** FIX 2: Safely destructure from the analysis object, providing a fallback. ***
...
`;
    ...
    const memoryNotes = [];
    // *** FIX 3: Use optional chaining ('?.') for maximum resilience against undefined data. ***
